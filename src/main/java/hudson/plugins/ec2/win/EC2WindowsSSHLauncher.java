...
public class EC2WindowsSSHLauncher extends EC2ComputerLauncher {
    ...
    protected void launchScript(EC2Computer computer, TaskListener listener) throws IOException,
        ...
        try {
            ...
            if (isBootstrapped) {
                ...
                if (!cleanupConn.authenticateWithPassword(computer.getRemoteAdmin(),computer.getNode().getAdminPassword().getPlainText())) {
            ...
            //TODO: errorcheck
            conn.exec("New-Item -ItemType Directory -Force -Path " + tmpDir, logger);
            ...
            if (initScript != null && initScript.trim().length() > 0) {
                ...
                scp.put(initScript.getBytes("UTF-8"), ".jenkins-init", tmpDir, "0700");
                Session sess = conn.openSession();
                sess.requestDumbPTY(); // so that the remote side bundles stdout
                // and stderr
                sess.execCommand(tmpDir + "/init.bat");
    ...
    private boolean bootstrap(EC2Computer computer, TaskListener listener, SlaveTemplate template) throws IOException,
        ...
        try {
            ...
            while (tries-- > 0) {
                ...
                try {
                    ...
                    isAuthenticated = bootstrapConn.authenticateWithPassword(computer.getRemoteAdmin(),computer.getNode().getAdminPassword().getPlainText());
                } catch (IOException e) {
        ...
        private Connection connectToSsh (EC2Computer computer, TaskListener listener, SlaveTemplate template) throws
        AmazonClientException,
            ...
            while (true) {
                try {
                    ...
                    conn.connect(new EC2WindowsSSHLauncher.ServerHostKeyVerifierImpl(computer, listener), slaveConnectTimeout, slaveConnectTimeout);
